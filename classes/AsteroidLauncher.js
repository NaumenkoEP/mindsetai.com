class AsteroidLauncher{
    ...
    static Projectile = class {
        ...
        handleCollision(){
            const playersSpawners = [];
            world.players.forEach((p) => {
                p.spawners.forEach((ps) => {
                    playersSpawners.push(ps)
                })
            });
            const playersAllies = [];
            world.players.forEach((p) => {
                p.allies.forEach((pa) => {
                    playersAllies.push(pa)
                })
            });
            
            this.owner.projectileTargets = [
                { type: "spawner", array: [...enemy.spawners, ...playersSpawners] },
                { type: "enemy", array: [...enemy.enemies, ...playersAllies] },
                { type: "player", array: world.players}
            ];
