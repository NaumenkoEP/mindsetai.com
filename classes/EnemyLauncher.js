class EnemyLauncher {
    ...
    update(camera){
        ...
        if(now - this.lastLaunch > this.launchRate && this.spawners.length < 10){
            this.launch();
            this.lastLaunch = now;
        }
    ...
    static Spawner = class {
        ...
        spawn(){
            ...
            let n = Math.random();
            if(world.secondPhase && n > 0.8) radius = 80 + Math.random() * 10;
    ...
    static Enemy = class {
        ...
        update(){
            ...
            let closestDistance = Infinity;
            let target = player; 
            const targets = [player, ...player.allies, ...player.spawners, ...player.towers]; 
        ...
        handleCollision(){
            this.targets = [
                ...
                { type: "tower", array: player.towers},
                ...
                for (let i = group.array.length - 1; i >= 0; i--) {
                    ...
                    if (distance < collisionDistance) {
                        ...
                        if (group.type === "spawner") {
                            ...
                            if (distance < spawnerCollisionDistance) target.hp--;
                            
                        } 
                        else if (group.type === "tower") {
                            const maxSpread = Math.max(...target.circles.map(c => c.spread));
                            const towerCollisionDistance = this.radius + maxSpread;
                            if (distance < towerCollisionDistance) target.hp--;
                        } 
                        else if(group.type === "ally"){
                            target.remove();
                            world.particleBlast(30, target.x, target.y, target.color, 3);
                        } 

                        else if (group.type === "player") {
                            const now = Date.now()
                            target.reduceHealth();
                            messageLogAnimation("Hit", "white", 1)
                            target.lastHitTime = now;
                            this.remove();
                            removed = true;
                        }

                        if(this.radius - 10 > 20) this.reduceRadius();
                        else {
