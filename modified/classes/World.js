class World {
    ...
    animate() {
        ...
        c.scale(localPlayer.zoomFactor, localPlayer.zoomFactor);
        c.translate(-canvas.width / 2, -canvas.height / 2);
        ...
        this.drawGridFor(localPlayer);


        // update entities
        this.particles.forEach((p) => {
            p.draw(localPlayer.camera);
            p.update();
        })
        enemy.update(localPlayer.camera);
        asteroid.update(localPlayer.camera)
        
        this.players.forEach((p) =>{
            p.spawners.forEach((s) => {
                s.draw(localPlayer.camera);
                s.update();
            });
            p.allies.forEach((a) => {
                a.draw(localPlayer.camera);
                a.update();
            });
            p.update(localPlayer.camera);
            p.drawFlash();
        });

        this.collectibles.forEach((c) => {
            c.draw(localPlayer.camera);
            c.update();
        })
        ...
        // Set to second phase;
        if (((this.friendlySquares / this.totalSquares) * 100).toFixed(1) > 90 && !this.secondPhase && singlePlayer){
            this.width = 10000;
            this.height = 10000;
            ...
            for(let i = 0; i < 5; i++){
                ...
                new EnemyLauncher.Spawner(enemy, c.x, c.y)
            }
            for(let i = 0; i < 20; i++){
                let c = {
                    x: Math.random() * 10000,
                ...
                new EnemyLauncher.Enemy(enemy, c.x, c.y, 70 + Math.random() * 10)
            }
            ...
            localPlayer.shootCoolDown = 100;
            localPlayer.rechargeStep = 0.5
    ...
    static Collectible = class {
        ...
        update(){
            ...
            for(let i = 0; i < this.owner.players.length; i++){
                const p = this.owner.players[i];
                const distance = Math.hypot(
                    p.x - this.x, 
                    p.y - this.y
                );

                if(this.type === "hp") additionalFactor = p.hp < 5;
                else additionalFactor = true;

                if(distance - this.radius - p.radius < 200 && additionalFactor){
